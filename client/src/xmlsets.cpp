...
{
	// load xml
	if(!xmlSettings.LoadFile("PizDoS Bot.xml"))
	{
		MessageBox(NULL, "Файл конфиг отсутствует", "Error", MB_ICONERROR);
		ExitProcess(0);
	}
	...
	if(rakSAMPElement)
	{
		...
		// get followplayer1
		strcpy(settings.szFollowingPlayerName1, (char *)rakSAMPElement->Attribute("Zaebat"));
		// get followplayer2
		strcpy(settings.szFollowingPlayerName2, (char *)rakSAMPElement->Attribute("pulse-weapon"));
		// get followplayer3
		strcpy(settings.szFollowingPlayerName3, (char *)rakSAMPElement->Attribute("killer_bot"));
		// get followplayer4
		strcpy(settings.szFollowingPlayerName4, (char *)rakSAMPElement->Attribute("Crazi_Teleporter"));
		// get followplayer4
		strcpy(settings.szFollowingPlayerName5, (char *)rakSAMPElement->Attribute("Zaebat2"));
...
int ReloadSettings()
{
	...
	{
		Log("Перезагрузка настроек");
		return 1;
	}

	Log("Конфиг бота отсутствует");
