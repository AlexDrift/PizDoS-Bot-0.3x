	...
	while(pkt = pRakClient->Receive())
	{
		...
		switch(packetIdentifier)
		{
			case ID_DISCONNECTION_NOTIFICATION:
				...
				{
					Log("[PizDoS Bot v0.3x] Подключение было закрыто сервером. Повторное подключение через %d секунд.", iReconnectTime / 1);
			...
			case ID_CONNECTION_BANNED:
				if (pRakClient == ::pRakClient)
				{
					Log("[PizDoS Bot v0.3x] Я забанен. Повторное подключение через %d секунд.", iReconnectTime / 1);
					resetPools(1, iReconnectTime);
				}
				break;			
			case ID_CONNECTION_ATTEMPT_FAILED:
				if (pRakClient == ::pRakClient)
				{
					Log("[PizDoS Bot v0.3x] Попытка подключения не удалась. Повторное подключение через %d секунд.", iReconnectTime / 1);
					resetPools(1, iReconnectTime);
				}
				break;
			case ID_NO_FREE_INCOMING_CONNECTIONS:
				if (pRakClient == ::pRakClient)
				{
					Log("[PizDoS Bot v0.3x] Сервер переполнен игроками. Повторное подключение через %d секунд.", iReconnectTime / 1);
					resetPools(1, iReconnectTime);
				}
				break;
			case ID_INVALID_PASSWORD:
				if (pRakClient == ::pRakClient)
				{
					Log("[PizDoS Bot v0.3x] Неверный пароль. Повторное подключение через %d секунд.", iReconnectTime / 1);
					resetPools(1, iReconnectTime);
				}
				break;
			case ID_CONNECTION_LOST:
				if (pRakClient == ::pRakClient)
				{
					Log("[PizDoS Bot v0.3x] Связь была потеряна. Повторное подключение через %d секунд.", iReconnectTime / 1);
					resetPools(1, iReconnectTime);
				}
				break;
