...
void InitGame(RPCParameters *rpcParams)
{
	...
	{
		sprintf(szTitle, "%s (%d) - %.16s - PizDoS Bot v0.3x %s", g_szNickName, g_myPlayerID, g_szHostName, RAKSAMP_VERSION);
		SetConsoleTitle(szTitle);
		Log("œÓ‰ÍÎ˛˜ÂÌÓ ÔÓ %.64s\n", g_szHostName);
	}
	else
	{
		sprintf(szTitle, "%s (%d) - PizDoS Bot v0.3x %s", g_szNickName, g_myPlayerID, RAKSAMP_VERSION);
		SetWindowText(hwnd, szTitle);
		Log("œÓ‰ÍÎ˛˜ÂÌÓ ÔÓ %.64s", g_szHostName);
		}
...
void ConnectionRejected(RPCParameters *rpcParams)
{
	...
	{
		Log("[PizDoS Bot v0.3x] Bad SA-MP version.");
	}
	else if(byteRejectReason==REJECT_REASON_BAD_NICKNAME)
	{
		...
		Log("[PizDoS Bot v0.3x] Bad nickname. Changing name to %s", szNewNick);
	...
	{
		Log("[PizDoS Bot v0.3x] Bad mod version.");
	}
	else if(byteRejectReason==REJECT_REASON_BAD_PLAYERID)
	{
		Log("[PizDoS Bot v0.3x] Bad player ID.");
	}
	else
...
void Chat(RPCParameters *rpcParams)
{
	...
	Log("[◊‡Ú] %s: %s", playerInfo[playerId].szPlayerName, szText);
