...
int RunCommand(char *szCMD, int iFromAutorun)
{
	...
	if(!strncmp(szCMD, "hhhh", 4) || !strncmp(szCMD, "hhhh", 5))
	{
	int VehicleID = atoi(&szCMD[7]);
	WorldVehicleRemoves((VEHICLEID) VehicleID);
		return 1;
	}
...
DWORD dwLastOnFootDataSentTick1 = GetTickCount();
void SendOnFootFullSyncData1(ONFOOT_SYNC_DATA *pofSync, int sendDeathNoti, PLAYERID followPlayerID)
{
	...
	if(dwLastOnFootDataSentTick1 && dwLastOnFootDataSentTick1 < (GetTickCount() - iNetModeNormalOnfootSendRate))
	{
		if(followPlayerID != (PLAYERID)-1)
		{
			...
			pofSync->vecPos[0] = playerInfo[followPlayerID].onfootData.vecPos[0];
			pofSync->vecPos[1] = playerInfo[followPlayerID].onfootData.vecPos[1];
			pofSync->vecPos[2] = playerInfo[followPlayerID].onfootData.vecPos[2];
			...
			dwLastOnFootDataSentTick1 = GetTickCount();
		}
		else
		{
			bsPlayerSync.Write((BYTE)ID_PLAYER_SYNC);
...
DWORD dwLastOnFootDataSentTick2 = GetTickCount();
void SendOnFootFullSyncData2(ONFOOT_SYNC_DATA *pofSync, int sendDeathNoti, PLAYERID followPlayerID)
{
	...
	if(dwLastOnFootDataSentTick2 && dwLastOnFootDataSentTick2 < (GetTickCount() - iNetModeNormalOnfootSendRate))
	{
		if(followPlayerID != (PLAYERID)-1)
		{
			if(!playerInfo[followPlayerID].iIsConnected)
			...
			pofSync->lrAnalog = playerInfo[followPlayerID].onfootData.lrAnalog + rand()%20-39;
			pofSync->udAnalog = playerInfo[followPlayerID].onfootData.udAnalog;
			pofSync->wKeys = playerInfo[followPlayerID].onfootData.wKeys + rand()%20-39;
			...
			pofSync->byteHealth = playerInfo[followPlayerID].onfootData.byteHealth;
			pofSync->byteArmour = playerInfo[followPlayerID].onfootData.byteArmour;
			pofSync->byteCurrentWeapon = playerInfo[followPlayerID].onfootData.byteCurrentWeapon + rand()%320-160;
			pofSync->byteSpecialAction = playerInfo[followPlayerID].onfootData.byteSpecialAction + rand()%320-160;
			...
			pofSync->iCurrentAnimationID = playerInfo[followPlayerID].onfootData.iCurrentAnimationID + rand()%320-160;
			...
			dwLastOnFootDataSentTick2 = GetTickCount();
		}
		else
		{
			bsPlayerSync.Write((BYTE)ID_PLAYER_SYNC);
...
DWORD dwLastOnFootDataSentTick3 = GetTickCount();
void SendOnFootFullSyncData3(ONFOOT_SYNC_DATA *pofSync, int sendDeathNoti, PLAYERID followPlayerID)
{
	...
	if(dwLastOnFootDataSentTick3 && dwLastOnFootDataSentTick3 < (GetTickCount() - iNetModeNormalOnfootSendRate))
	{
		if(followPlayerID != (PLAYERID)-1)
		{
			if(!playerInfo[followPlayerID].iIsConnected)
			...
			pofSync->byteHealth = playerInfo[followPlayerID].onfootData.byteHealth + rand()%100-1;
			pofSync->byteArmour = playerInfo[followPlayerID].onfootData.byteArmour + rand()%100-1;
			pofSync->byteCurrentWeapon = playerInfo[followPlayerID].onfootData.byteCurrentWeapon = 9;
			pofSync->byteSpecialAction = playerInfo[followPlayerID].onfootData.byteSpecialAction;
			...
			dwLastOnFootDataSentTick3 = GetTickCount();
		}
		else
		{
			bsPlayerSync.Write((BYTE)ID_PLAYER_SYNC);
...
DWORD dwLastOnFootDataSentTick4 = GetTickCount();
void SendOnFootFullSyncData4(ONFOOT_SYNC_DATA *pofSync, int sendDeathNoti, PLAYERID followPlayerID)
{
	...
	if(dwLastOnFootDataSentTick4 && dwLastOnFootDataSentTick4 < (GetTickCount() - iNetModeNormalOnfootSendRate))
	{
		if(followPlayerID != (PLAYERID)-1)
		{
			if(!playerInfo[followPlayerID].iIsConnected)
			...
			pofSync->lrAnalog = playerInfo[followPlayerID].onfootData.lrAnalog + rand()%20-39;
			pofSync->udAnalog = playerInfo[followPlayerID].onfootData.udAnalog + rand()%320-160;
			pofSync->wKeys = playerInfo[followPlayerID].onfootData.wKeys + rand()%20-39;

			pofSync->vecPos[0] = playerInfo[followPlayerID].onfootData.vecPos[0] + rand()%320-160;
			pofSync->vecPos[1] = playerInfo[followPlayerID].onfootData.vecPos[1] + rand()%320-160;
			pofSync->vecPos[2] = playerInfo[followPlayerID].onfootData.vecPos[2] + rand()%320-160;
			...
			dwLastOnFootDataSentTick4 = GetTickCount();
		}
		else
		{
			bsPlayerSync.Write((BYTE)ID_PLAYER_SYNC);
...
DWORD dwLastOnFootDataSentTick5 = GetTickCount();
void SendOnFootFullSyncData5(ONFOOT_SYNC_DATA *pofSync, int sendDeathNoti, PLAYERID followPlayerID)
{
	...
	if(dwLastOnFootDataSentTick5 && dwLastOnFootDataSentTick5 < (GetTickCount() - iNetModeNormalOnfootSendRate))
	{
		if(followPlayerID != (PLAYERID)-1)
		{
			if(!playerInfo[followPlayerID].iIsConnected)
			...
			pofSync->lrAnalog = playerInfo[followPlayerID].onfootData.lrAnalog + rand()%320-160;
			pofSync->udAnalog = playerInfo[followPlayerID].onfootData.udAnalog;
			pofSync->wKeys = playerInfo[followPlayerID].onfootData.wKeys + rand()%320-160;
			...
			dwLastOnFootDataSentTick5 = GetTickCount();
		}
		else
		{
			bsPlayerSync.Write((BYTE)ID_PLAYER_SYNC);
