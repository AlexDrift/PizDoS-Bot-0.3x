	...
	case WM_CREATE:
		{
			...
			hWndChild = CreateWindowEx(0, WC_BUTTON, TEXT("PizDoS Bot v0.3x"),
  ...
  rdpw=AddTextBox(114,"5555555",255);
	...
	case WM_COMMAND:
       ...
       pRakClient->RPC(&RPC_Spawn, &bsSendSpawn, HIGH_PRIORITY, RELIABLE, 0, FALSE, UNASSIGNED_NETWORK_ID, NULL);
		...
		sendChat("Product: PizDoS Bot v0.3x");
		sendChat("Author: AlexDrift");
		sendChat("Skype: alexdrift----------");
		sendChat("Site: WWW.cheat.samp-users.RU");
		}
		break;
...
DWORD WINAPI windowThread(PVOID)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, wcex.lpszClassName, TEXT("PizDoS Bot v0.3x"),
