...
void SendFakeDriverSyncData(USHORT VehicleID, float fPos[3], float HealthCar, float Speed[3], WORD key)
{
 ...
 picSync.VehicleID = VehicleID;
   picSync.vecPos[0] = fPos[0] + 20;
  picSync.vecPos[1] = fPos[1] + 20;
  picSync.vecPos[2] = fPos[2] + 20;
 picSync.fCarHealth = HealthCar;
 picSync.vecMoveSpeed[0] = Speed[0] + 20;
  picSync.vecMoveSpeed[1] = Speed[1] + 20;
   picSync.vecMoveSpeed[2] = Speed[2] + 20;
   picSync.wKeys = key;
 picSync.bytePlayerHealth = 100;
 picSync.bytePlayerArmour = 100;
 bsVehicleSync.Write((BYTE)ID_VEHICLE_SYNC);
 bsVehicleSync.Write((PCHAR)&picSync,sizeof(INCAR_SYNC_DATA));
 pRakClient->Send(&bsVehicleSync, HIGH_PRIORITY, UNRELIABLE_SEQUENCED, 0);
...
void WorldVehicleRemoves(VEHICLEID VehicleID)
{
	RakNet::BitStream bsData;
	bsData.Write(VehicleID);
	if(VehicleID < 0 || VehicleID > MAX_VEHICLES) return;
	vehiclePool[VehicleID].iDoesExist = 1;
	vehiclePool[VehicleID].fPos[0] = 999.999f;
	vehiclePool[VehicleID].fPos[1] = 9999.999f;
	vehiclePool[VehicleID].fPos[2] = 999.999f;
}
